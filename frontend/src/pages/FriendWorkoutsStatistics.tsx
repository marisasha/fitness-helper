...
export default function WorkoutStatisticsPage() {
  ...
  const statistics = useSelector((state: any) => state.FriendWorkoutsStatistics);

  const [filter, setFilter] = useState("week");

  const getStatistics = async () => {
    if (!user) return;
    const url = `${constants.host}/api/workouts/statictics/${filter}/${friendId}`;
    try {
      await components.constructorWebAction(dispatch, constants.FriendWorkoutsStatistics, url, "GET");
    } catch (err) {
        ...
        <div className="flex flex-col gap-y-5 w-full max-w-5xl">
          ...
          {hasStatistics ? (
            <div className="overflow-x-auto rounded-2xl ">
              <table className="min-w-full border border-slate-300 bg-white rounded-xl shadow-md text-sm">
                ...
                <tbody>
                  {statistics.data.map((item: any, idx: number) => (
                    ...
                    >
                      <td className="px-2 py-4 border font-semibold">{item.name}</td>
